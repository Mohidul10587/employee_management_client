...
const ClockIn = () => {
    ...
    const fetchOfficeLocations = async () => {
      try {
        const response = await axios.get(
          `https://test2.notebookprokash.com/employee/office_location`
        );
    ...
    if (location === "home") {
      try {
        const response = await axios.post(
          `https://test2.notebookprokash.com/employee/employee_clockin/${id}`,
          {
            work_from_type: "Home",
          }
        );
  ...
  const checkOfficeLocation = (userLatitude, userLongitude) => {
    for (const officeLocation of officeLocations) {
      const officeLatitude = parseFloat(officeLocation.latitude);
      const officeLongitude = parseFloat(officeLocation.longitude);
      const YOUR_TOLERANCE = 5000;
  ...
  const handleClockOut = async () => {
    ...
    try {
      const response = await axios.post(
        `https://test2.notebookprokash.com/employee/employee_clockout/${id}`
      );
